...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Revenu total', value: `${stats?.totalRevenue?.toFixed(0)} XAF`, color: 'teal' },
  ]
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {cards.map((card, i) => (
            <div key={i} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <p className={`text-2xl font-bold ${colorMap[card.color].split(' ')[1]}`}>
                {card.value ?? '—'}
              </p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            ...
            <p className="text-3xl font-bold text-blue-600">
              {stats?.averageCourseCompletion?.toFixed(1)}%
            </p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <h3 className="font-semibold text-gray-700 mb-3">Taux de réussite Quiz</h3>
            <p className="text-3xl font-bold text-green-600">
              {stats?.averageQuizPassRate?.toFixed(1)}%
            </p>
          </div>
          <div className="flex gap-4 mt-6">
            <a href="/admin/categories" className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-blue-700">
              Gérer les catégories
            </a>
            <a href="/admin/courses" className="bg-purple-600 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-purple-700">
              Valider les cours
            </a>
          </div>
        </div>
